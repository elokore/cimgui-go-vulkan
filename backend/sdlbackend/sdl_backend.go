...
// #cgo darwin LDFLAGS: -framework CoreHaptics -framework ForceFeedback
...
var _ backend.Backend[SDLWindowFlags] = &SDLBackend{}
...
func (b *SDLBackend) Run(loop func()) {
	b.loop = loop
	C.igSDLRunLoop(b.handle(), C.VoidCallback(backend.LoopCallback()), C.VoidCallback(backend.BeforeRender()), C.VoidCallback(backend.AfterRender()), C.VoidCallback(backend.BeforeDestroyContext()))
}
...
func (b *SDLBackend) CreateTexture(pixels unsafe.Pointer, width, height int) imgui.TextureRef {
...
func (b *SDLBackend) CreateTextureRgba(img *image.RGBA, width, height int) imgui.TextureRef {
	tex := C.igCreateTexture((*C.uchar)(&(img.Pix[0])), C.int(width), C.int(height))
	return *imgui.NewTextureRefTextureID(*imgui.NewTextureIDFromC(&tex))
}

func (b *SDLBackend) DeleteTexture(id imgui.TextureRef) {
